<template>
  <el-dialog :title="title" :visible.sync="show" width="85%" :close-on-click-modal="false" append-to-body :before-close="close">
    ...
    <pagination v-show="total>0" :total="total" :page.sync="queryParams.pageNum" :limit.sync="queryParams.pageSize" @pagination="noDeptList" />
...
<script>
import { noDeptList } from "@/api/caseManage/createCase";
...
export default {
  ...
  methods: {
    ...
    // 获取企业列表
    noDeptList() {
      noDeptList(this.queryParams).then((res) => {
